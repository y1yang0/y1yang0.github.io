...
.q-label-tr { top: 20px; right: 20px; color: rgba(34, 197, 94, 0.4); font-size: 24px; }
.q-label-tl { top: 20px; left: 20px; color: rgba(239, 68, 68, 0.4); font-size: 24px; }
.q-label-bl { bottom: 20px; left: 20px; color: rgba(107, 114, 128, 0.4); font-size: 24px; }
.q-label-br { bottom: 20px; right: 20px; color: rgba(59, 130, 246, 0.4); font-size: 24px; }
...
<div id="app">
    ...
    <div class="quadrant-chart-container">
        <div class="quadrant-chart-header">
            文治武功象限图
            <span style="font-size:10px;font-weight:400;color:#6b7280;margin-left:8px;">X轴: 文治 (治国+民生+创新) &nbsp;&nbsp; Y轴: 武功 (军事+魅力)</span>
        </div>
        <div class="quadrant-chart-body" id="quadrantChartBody">
            <div class="q-label-tr quadrant-label">文武双全</div>
            <div class="q-label-tl quadrant-label">穷兵黩武</div>
            <div class="q-label-bl quadrant-label">昏庸无能</div>
            <div class="q-label-br quadrant-label">仁弱守成</div>
        </div>
    </div>
</div>
...
function renderQuadrantChart() {
    const container = document.getElementById('quadrantChartBody');
    const tooltip = document.getElementById('chartTooltip');
    if (!container) return;
    ...
    // Data preparation
    const data = emperors.map(e => ({
        ...e,
        civil: (e.scores.govern + e.scores.welfare + e.scores.innovation) / 3,
        military: e.scores.military
    }));

    // Dynamic Range
    const xVals = data.map(e => e.civil);
    const yVals = data.map(e => e.military);
    const minX = Math.min(...xVals), maxX = Math.max(...xVals);
    const minY = Math.min(...yVals), maxY = Math.max(...yVals);
    
    // Scales with 10% padding on data range to avoid edge clipping
    const xRange = maxX - minX || 1;
    const yRange = maxY - minY || 1;
    const xLow = minX - xRange * 0.05;
    const xHigh = maxX + xRange * 0.05;
    const yLow = minY - yRange * 0.05;
    const yHigh = maxY + yRange * 0.05;

    const toX = v => pad + ((v - xLow) / (xHigh - xLow)) * (W - 2 * pad);
    const toY = v => H - pad - ((v - yLow) / (yHigh - yLow)) * (H - 2 * pad);

    // Averages for Quadrants
    const avgX = xVals.reduce((a, b) => a + b, 0) / xVals.length;
    const avgY = yVals.reduce((a, b) => a + b, 0) / yVals.length;
    
    const crossX = toX(avgX);
    const crossY = toY(avgY);

    // HTML buffer - clear existing dots but keep labels
    // We'll rebuild content. Save labels first? 
    // Easier to just append dots and axes to a clean slate or overlay.
    // The existing labels are in HTML. We should position them based on crosshair?
    // No, keep labels in corners.
    
    let axisHtml = '';

    // Axis Lines (Crosshair at average)
    axisHtml += `<div style="position:absolute;left:${pad}px;right:${pad}px;top:${crossY}px;height:1px;background:#e5e7eb;z-index:1;border-top:1px dashed #9ca3af;"></div>`; // X axis
    axisHtml += `<div style="position:absolute;top:${pad}px;bottom:${pad}px;left:${crossX}px;width:1px;background:#e5e7eb;z-index:1;border-left:1px dashed #9ca3af;"></div>`; // Y axis
    
    // Axis Labels
    axisHtml += `<div class="q-axis-label" style="right:${pad}px;top:${crossY + 6}px;">文治 &rarr;</div>`;
    axisHtml += `<div class="q-axis-label" style="left:${crossX + 6}px;top:${pad}px;">&uarr; 武功</div>`;
    axisHtml += `<div class="q-axis-label" style="left:${crossX + 6}px;top:${crossY + 6}px;background:rgba(255,255,255,0.8);padding:2px;border-radius:4px;">均值</div>`;

    // Render Dots
    let dotsHtml = '';
    data.forEach(e => {
        const x = toX(e.civil);
        const y = toY(e.military);
        const color = getDynastyColor(e.dynasty);
        
        dotsHtml += `<div class="q-dot" style="left:${x}px;top:${y}px;background:${color};" data-name="${e.name}"></div>`;
    });

    // Append to container (keep existing static labels)
    // Create a wrapper for dynamic content to easily clear/update
    let dynamicLayer = container.querySelector('.dynamic-layer');
    if (!dynamicLayer) {
        dynamicLayer = document.createElement('div');
        dynamicLayer.className = 'dynamic-layer';
        dynamicLayer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;z-index:10;pointer-events:none;';
        container.appendChild(dynamicLayer);
    }
    dynamicLayer.innerHTML = axisHtml + dotsHtml;
    
    // Enable pointer events for dots
    dynamicLayer.querySelectorAll('.q-dot').forEach(el => el.style.pointerEvents = 'auto');

    // Tooltip Logic
    dynamicLayer.querySelectorAll('.q-dot').forEach(dot => {
        dot.addEventListener('mouseenter', (ev) => {
            const name = dot.dataset.name;
            const e = data.find(emp => emp.name === name);
            const civil = Math.round(e.civil);
            const martial = e.military;
            
            tooltip.innerHTML = `<b>${e.name}</b><br>${e.dynasty}<br>文治: <b>${civil}</b><br>武功: <b>${martial}</b>`;
            tooltip.style.opacity = '1';
...
(function init() {
    ...
    // Render quadrant chart
    setTimeout(renderQuadrantChart, 100); // Wait for layout
