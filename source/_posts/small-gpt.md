...
残差、Pre-Norm、Attention、梯度裁剪，该有的基本上都有了，模型参数量是74831872(75M)，数据集使用金庸的全部小说，loss1.x左右，smallgpt最终会在2~词左右犹豫并输出，从指标上看应该可以了，但是推理效果非常不理想：
...
白忙活一场，只能重头再来，先训练tokenizer再训练模型。这一次训练了10个epoch，loss 3.x，e^3.x≈20，说明smallgpt基本在20~30词左右犹豫并输出。中文分词优化后，推理效果如下：
...
可能的解释是，token embedding和out在大模型充分学习的情况下，两者相似，但是对于我这个数据量小+模型参数小的情况下，两者不相似？本来smallgpt模型在token embedding层学习到怎么将token映射到embedding vector，比如杨过和小龙女到相似空间位置，降龙十八掌和打狗棒法到相似空间位置，out层学习到怎么根据上下文向量预测最合适的下一个词，但是权重共享后，两个任务共享一个参数，手忙脚乱了？
...
现在我根据scaling law调整了smallgpt配置，参数规模是29.7m，数据集是5.6m：

```shell
@@ SmallGPT Configuration:
@@    Device: cuda
...
之前smallgpt用的是绝对位置编码，就是tokenEmbedding+posEmbedding。假设有下面的句子
...
smallgpt搞到目前这个程度，优化模型本身来提升智慧的方法已经比较有限了，后面可能需要从数据上面下手。
我还是不希望增加太多数据，就想看金庸15本书训练的smallgpt到底能达到什么程度，所以在不增加数据的情况下，我实现了sft，用大模型生成了1700+条指令对：
...
SFT确实好像教会了smallgpt回答问题，比如问`XX是谁？`，它知道回答`XX是XX`。但是除了之外，推理效果基本上没提升，毕竟数据太少了，只有1700+。

搞到这里，我感觉我应该放弃SFT，重点还是关注预训练，我的目标是让它流畅续写武侠小说，语法正确，叙事连贯，不胡说八道，张冠李戴都无所谓。而SFT的目标是让模型懂得输出格式，这不是我的目标

## Repeat Penalty

虽然频率很低，但是smallgpt偶尔还是会复读。这是小模型的通病，我加上了复读机惩罚，其实就是模型算出来logits后，先向前看一些，比如看5个最近生成的词，然后在logits词汇表中寻找这些最近生成的词，给他们一个缩放系数，主动降低他们的logits得分，避免复读。整体实现如下：
...
@@ DebugInput: 韦小宝和双儿
@@ Attn#0:
   韦小宝和 ['菩萨(3.4)', '求饶(3.4)', '腮(3.3)', '中的一(3.2)', '之仇(3.2)']
   双儿 ['日上(3.9)', '蝶(3.5)', '蘸(3.5)', '上涌(3.3)', '南来(3.3)']
@@ FFN#0:
   韦小宝和 ['菩萨(3.6)', '求饶(3.3)', '祈(3.3)', '求生(3.1)', '幻(3.0)']
   双儿 ['俏脸(3.9)', '蘸(3.8)', '噗(3.6)', '日上(3.6)', '的脸上(3.5)']
@@ Attn#1:
   韦小宝和 ['良(3.9)', '祈(3.9)', '之仇(3.5)', '求饶(3.4)', '之能(3.4)']
   双儿 ['噗(4.5)', '睡去(4.1)', '居住(4.0)', '往外(3.9)', '跟来(3.9)']
@@ FFN#1:
   韦小宝和 ['良(3.8)', '祈(3.8)', '波(3.5)', '幻(3.2)', '悦(3.2)']
   双儿 ['噗(4.6)', '的脸上(4.4)', '往外(4.1)', '居住(3.9)', '的一个(3.9)']
@@ Attn#2:
   韦小宝和 ['祈(4.3)', '良(4.1)', '蹈(3.8)', '许(3.8)', '不和(3.7)']
   双儿 ['噗(4.7)', '的脸上(4.4)', '居住(4.4)', '任意(4.2)', '跟来(4.1)']
@@ FFN#2:
   韦小宝和 ['良(4.0)', '祈(3.8)', '不和(3.7)', '菩萨(3.6)', '求生(3.5)']
   双儿 ['噗(5.0)', '的脸上(4.5)', '服侍(4.2)', '往外(4.1)', '潜心(4.1)']
```

在Transformer前三层，smallgpt看到韦小宝最先想到`求饶，菩萨，求生`，看到双儿最先想到`噗，脸上`。
而且对`噗`一开始就很有信心。

```shell
@@ Attn#3:
   韦小宝和 ['身(4.6)', '良(4.4)', '常(4.0)', '敌(3.9)', '枝(3.9)']
   双儿 ['噗(4.6)', '的脸上(4.0)', '服侍(4.0)', '潜心(3.9)', '跟(3.9)']
@@ FFN#3:
   韦小宝和 ['身(4.8)', '非(4.3)', '成(4.3)', '常(4.1)', '枝(4.0)']
   双儿 ['跟(4.8)', '的(4.3)', '把(4.3)', '走进房来(4.3)', '跟在(4.3)']
@@ Attn#4:
   韦小宝和 ['议(5.4)', '声(5.2)', '身(4.9)', '不成(4.8)', '悦(4.8)']
   双儿 ['的(5.0)', '走进房来(5.0)', '跟(4.8)', '把(4.7)', '离(4.6)']
@@ FFN#4:
   韦小宝和 ['声(5.3)', '身(5.1)', '祈(5.0)', '不(4.9)', '非(4.8)']
   双儿 ['的(5.2)', '跟(5.2)', '在(4.9)', '给(4.9)', '和(4.9)']
@@ Attn#5:
   韦小宝和 ['声(6.2)', '祈(6.2)', '远(5.8)', '不着(5.8)', '不得(5.7)']
   双儿 ['的(6.4)', '在(5.7)', '跟(5.6)', '离(5.6)', '和(5.5)']
@@ FFN#5:
   韦小宝和 ['声(6.4)', '祈(6.3)', '不着(6.2)', '议(6.1)', '不(5.7)']
   双儿 ['的(6.6)', '在(5.9)', '下落(5.8)', '离(5.8)', '，(5.7)']
```
中间三层，logits稳步提升，模型对自己的答案越来越有信心。但是不知道为什么我感觉这个阶段得出的token质量比之前差一些？

```shell
@@ Attn#6:
   韦小宝和 ['祈(8.4)', '声(8.1)', '议(7.6)', '，(7.4)', '良(7.3)']
   双儿 ['的(8.1)', '，(7.4)', '、(7.4)', '在(7.3)', '和(7.3)']
@@ FFN#6:
   韦小宝和 ['祈(8.3)', '声(8.0)', '议(7.8)', '良(7.4)', '，(7.3)']
   双儿 ['，(9.0)', '的(8.8)', '在(8.3)', '、(8.0)', '在一起(7.9)']
@@ Attn#7:
   韦小宝和 ['双方(9.7)', '声(9.6)', '，(9.5)', '祈(9.2)', '议(8.9)']
   双儿 ['，(10.9)', '的(10.3)', '在(9.5)', '、(9.5)', '在一起(9.4)']
@@ FFN#7:
   韦小宝和 ['，(10.4)', '双方(9.5)', '不得(9.3)', '声(9.0)', '的(9.0)']
   双儿 ['，(11.1)', '的(10.2)', '在(9.9)', '、(9.7)', '在一起(9.6)']
@@ FinalNorm:
   韦小宝和 ['，(7.2)', '双方(6.8)', '不得(6.5)', '声(6.5)', '的(6.3)']
   双儿 ['，(9.0)', '的(8.2)', '在(8.0)', '、(7.8)', '在一起(7.8)']
@@ DebugOutput: 在一起
```

最后两层，logits稳步提升，它认为最稳妥的是标点符号，但是最后由于温度采样加top-p，模型没选择最有信心的标点符号，而是选择了一个还不错的`在一起`。
...
| Inspect Model Internals | 辅助功能，可视化模型的思考过程 |